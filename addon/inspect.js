...
const TRUSTPILOT_CONFIG = TRUSTPILOT_MVP_CONFIG;
...
class Model {
  ...
  getTrustpilotTemplates() {
    return asArray(this.trustpilotConfig?.templates)
      ...
      .map(template => ({
        ...
        templateId: firstNonEmpty(template.templateId)
      }));
  }
  getTrustpilotOperatorKey() {
    return firstNonEmpty(this.trustpilotOperator?.id, this.trustpilotOperator?.email, this.trustpilotOperator?.name);
  }
  getTrustpilotOperatorRouting() {
    let operatorKey = this.getTrustpilotOperatorKey();
    if (!operatorKey) {
      return null;
    }
    return this.trustpilotConfig?.operators?.[operatorKey] || null;
  }
  getTrustpilotBusinessUserId() {
    let routing = this.getTrustpilotOperatorRouting() || {};
    return firstNonEmpty(
      routing.trustpilotBusinessUserId,
  ...
  buildTrustpilotPreview(templateId = "") {
    ...
    return {
      ...
      operatorEmail: firstNonEmpty(this.trustpilotOperator?.email),
      communicationId,
      tags
    };
  }
  trustpilotMissingData(templateId = "") {
    let preview = this.buildTrustpilotPreview(templateId);
    let missing = [];
    let grantType = firstNonEmpty(this.trustpilotConfig?.auth?.grantType, "client_credentials").toLowerCase();
    let trustpilotBusinessUserId = this.getTrustpilotBusinessUserId();
    if (!this.trustpilotConfig?.auth?.url) missing.push("auth.url");
    if (!this.trustpilotConfig?.auth?.token) missing.push("auth.token");
    if (!this.trustpilotConfig?.auth?.secret) missing.push("auth.secret");
    if (!this.trustpilotConfig?.invitation?.url) missing.push("invitation.url");
    if (grantType === "client_credentials" && !trustpilotBusinessUserId) missing.push("invitation.businessUserId/x-business-user-id");
  ...
  refreshTrustpilotContext() {
    if (!this.isTrustpilotEnabledForCurrentRecord()) {
      return;
    }
  ...
  loadTrustpilotOperatorContext() {
    this.spinFor(
      "retrieving trustpilot operator",
      sfConn.rest("/services/data/v" + apiVersion + "/chatter/users/me").then(res => {
        this.trustpilotOperator = {
          id: firstNonEmpty(res.id),
          name: firstNonEmpty(res.name, res.displayName),
          email: firstNonEmpty(res.email)
        };
      })
    );
  }
  loadTrustpilotContactContext() {
    let contactId = firstNonEmpty(this.recordData?.ContactId);
    if (!contactId) {
      this.trustpilotContact = null;
      return;
    }
    let query = "SELECT Id, Name, FirstName, LastName, Email FROM Contact WHERE Id = '" + contactId + "' LIMIT 1";
    this.spinFor(
  ...
  resolveTrustpilotInvitationUrl() {
    let invitationUrl = firstNonEmpty(this.trustpilotConfig?.invitation?.url);
    let businessUnitId = firstNonEmpty(this.trustpilotConfig?.invitation?.businessUnitId);
    if (businessUnitId) {
      invitationUrl = invitationUrl.replace("{businessUnitId}", encodeURIComponent(businessUnitId));
    }
    return invitationUrl;
  }
  getTrustpilotTokenCacheKey() {
    return this.sfHost + "_trustpilot_token_cache";
  }
  readTrustpilotTokenFromCache() {
    let raw = localStorage.getItem(this.getTrustpilotTokenCacheKey());
    if (!raw) {
      return null;
    }
    try {
      let cached = JSON.parse(raw);
      if (!cached || !cached.accessToken || !cached.expiresAt) {
        return null;
      }
      return cached;
    } catch (e) {
  ...
  saveTrustpilotTokenToCache(accessToken, expiresAt) {
    localStorage.setItem(this.getTrustpilotTokenCacheKey(), JSON.stringify({accessToken, expiresAt}));
  }
  clearTrustpilotTokenCache() {
    localStorage.removeItem(this.getTrustpilotTokenCacheKey());
  }
  computeTrustpilotTokenExpiry(responseBody) {
    const now = Date.now();
    const expiresInSeconds = Number(responseBody?.expires_in);
    if (Number.isFinite(expiresInSeconds) && expiresInSeconds > 0) {
      return now + expiresInSeconds * 1000;
    }
    const expiresAtRaw = responseBody?.expires_at || responseBody?.expiresAt;
    if (expiresAtRaw) {
      let expiresAt = new Date(expiresAtRaw).getTime();
      if (Number.isFinite(expiresAt) && expiresAt > now) {
        return expiresAt;
      }
    }
    let fallbackTtlSeconds = Number(this.trustpilotConfig?.auth?.defaultTtlSeconds);
    if (!Number.isFinite(fallbackTtlSeconds) || fallbackTtlSeconds <= 0) {
      fallbackTtlSeconds = 3300; // 55 minutes
    }
    return now + fallbackTtlSeconds * 1000;
  }
  getTrustpilotRefreshSkewMs() {
    let skewSeconds = Number(this.trustpilotConfig?.auth?.refreshSkewSeconds);
    if (!Number.isFinite(skewSeconds) || skewSeconds < 0) {
      skewSeconds = 120;
    }
    return skewSeconds * 1000;
  }
  async fetchTrustpilotAccessToken() {
    const now = Date.now();
    const refreshSkewMs = this.getTrustpilotRefreshSkewMs();
    const cached = this.readTrustpilotTokenFromCache();
    if (cached && now < (cached.expiresAt - refreshSkewMs)) {
      return cached.accessToken;
    }

    let authConfig = this.trustpilotConfig?.auth || {};
    let grantType = firstNonEmpty(authConfig.grantType, "client_credentials");
    let body = new URLSearchParams();
    body.set("grant_type", grantType);
    body.set("client_id", authConfig.token);
    body.set("client_secret", authConfig.secret);
    if (authConfig.scope) {
      body.set("scope", authConfig.scope);
    }
    let response = await fetch(authConfig.url, {
      method: "POST",
    ...
    if (!response.ok) {
      if (response.status === 401 || response.status === 403) {
        this.clearTrustpilotTokenCache();
      }
      throw new Error("Token request failed (" + response.status + "): " + (responseBody.error_description || responseBody.error || response.statusText));
    }
    let token = firstNonEmpty(responseBody.access_token, responseBody.token, responseBody.bearerToken);
    if (!token) {
      throw new Error("Token response without access token");
    }
    this.saveTrustpilotTokenToCache(token, this.computeTrustpilotTokenExpiry(responseBody));
    return token;
  }
  async sendTrustpilotInvitation(templateId) {
    let missing = this.trustpilotMissingData(templateId);
    if (missing.length > 0) {
      this.trustpilotStatus = {type: "error", message: "Missing required config/data: " + missing.join(", ")};
      this.didUpdate();
      return;
    }

    let preview = this.buildTrustpilotPreview(templateId);
    this.trustpilotSendingTemplateId = templateId;
    this.trustpilotStatus = null;
    this.didUpdate();
    ...
    try {
      ...
      // Remove undefined keys so Trustpilot receives a clean payload.
      Object.keys(payload).forEach(key => payload[key] === undefined && delete payload[key]);

      let response = await fetch(this.resolveTrustpilotInvitationUrl(), {
        method: "POST",
        headers: {
          "Authorization": "Bearer " + accessToken,
          "Content-Type": "application/json",
          ...(trustpilotBusinessUserId ? {"x-business-user-id": trustpilotBusinessUserId} : {})
        },
      ...
      if (!response.ok) {
        ...
        throw new Error("Invitation request failed (" + response.status + "): " + JSON.stringify(responseBody));
      }
      this.trustpilotStatus = {
        type: "success",
        message: "Invitation sent successfully (" + preview.templateName + ").",
        responseBody
      };
    } catch (err) {
      this.trustpilotStatus = {
...
class App extends React.Component {
  constructor(props) {
    ...
    this.onSendTrustpilotTemplate = this.onSendTrustpilotTemplate.bind(this);
  ...
  onSendTrustpilotTemplate(templateId, e) {
    e.preventDefault();
    let {model} = this.props;
    model.sendTrustpilotInvitation(templateId);
  }
  ...
  render() {
    ...
    return (
      h("div", {onClick: this.handleClick},
        ...
        model.isTrustpilotEnabledForCurrentRecord() ? h(TrustpilotPanel, {
          model,
          onSendTrustpilotTemplate: this.onSendTrustpilotTemplate
        }) : null,
...
class TrustpilotPanel extends React.Component {
  render() {
    let {model, onSendTrustpilotTemplate} = this.props;
    let templates = model.getTrustpilotTemplates();
    ...
    return h("div", {className: "trustpilot-panel"},
      ...
      h("div", {className: "trustpilot-panel-subtitle"}, "Verifica los datos antes de enviar la invitacion."),
      h("div", {className: "trustpilot-panel-buttons"},
        templates.length === 0 ? h("span", {className: "trustpilot-inline-warning"}, "No templates configured in trustpilot-config.js") : null,
        templates.map(template =>
          h("button", {
            key: template.key,
            className: "button button-brand trustpilot-send-button",
            disabled: model.trustpilotSendingTemplateId !== null,
            onClick: e => onSendTrustpilotTemplate(template.templateId, e),
            title: "Send Trustpilot invitation using this template"
          }, model.trustpilotSendingTemplateId === template.templateId ? "Sending..." : "Enviar resena: " + template.label)
        )
      ),
